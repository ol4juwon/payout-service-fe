...
const UserViewRight = ({ id, tab, userDetails }) => {
  // ** State
  ...
  const handleChange = (event, value) => {
    ...
    router
      .push({
        pathname: `/user/view/${id}/${value.toLowerCase()}`
      })
  ...
  return (
    <TabContext value={activeTab}>
      ...
      >
        ...
        <Tab value='benficiary' label='Beneficiary' icon={<Icon fontSize='1.125rem' icon='tabler:lock' />} />
      </TabList>
      <Box sx={{ mt: 6 }}>
        {isLoading ? (
          ...
          <>
            <TabPanel sx={{ p: 0 }} value='account'>
              <UserViewAccount userDetails={userDetails} />
            </TabPanel>
            <TabPanel sx={{ p: 0 }} value='security'>
              <UserViewSecurity />
            </TabPanel>
            <TabPanel sx={{ p: 0 }} value='benficiary'>
              <UserViewBilling id={id} user={userDetails} />
            </TabPanel>
